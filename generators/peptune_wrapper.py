...
if not tr2d2_path.exists() or not (tr2d2_path / "diffusion.py").exists():
...
class PepMDLMWrapper(BaseGenerator):
    ...
    def __init__(self, model_path: str, config_path: Optional[str] = None, 
        ...
        if tokenizer_vocab is None:
            for p in [lapep_root / "lapep" / "tr2d2" / "tokenizer" / "new_vocab.txt",
        ...
        if tokenizer_splits is None:
            for p in [lapep_root / "lapep" / "tr2d2" / "tokenizer" / "new_splits.txt",
                      lapep_root / "tr2d2-pep" / "tokenizer" / "new_splits.txt",
                      lapep_root.parent / "TR2-D2" / "tr2d2-pep" / "tokenizer" / "new_splits.txt"]:
                if p.exists():
                    tokenizer_splits = p
                    break
        
        self.tokenizer_vocab = str(tokenizer_vocab) if tokenizer_vocab and Path(tokenizer_vocab).exists() else None
        self.tokenizer_splits = str(tokenizer_splits) if tokenizer_splits and Path(tokenizer_splits).exists() else None
        
        if config_path is None:
            for p in [lapep_root / "lapep" / "tr2d2" / "configs" / "peptune_config.yaml",
                      lapep_root / "tr2d2-pep" / "configs" / "peptune_config.yaml",
                      lapep_root.parent / "TR2-D2" / "tr2d2-pep" / "configs" / "peptune_config.yaml"]:
                if p.exists():
                    config_path = p
                    break
    ...
    def _load_model(self):
        ...
        if not (self.tokenizer_vocab and self.tokenizer_splits):
            self.model = None
            return
        
        vocab_path = Path(self.tokenizer_vocab)
        splits_path = Path(self.tokenizer_splits)
        if not (vocab_path.exists() and splits_path.exists()):
            self.model = None
            return
        
        self.tokenizer = SMILES_SPE_Tokenizer(str(vocab_path), str(splits_path))
    ...
    def sample_unconditioned(self, num_samples: int = 1, seq_length: int = 100, 
        ...
        with torch.no_grad():
            ...
            for batch_start in range(0, num_samples, batch_size):
                ...
                samples = self.model._sample(num_steps=num_steps, eps=eps, x_input=None)
    ...
    def proposal_probability(self, x_prime: str, x: str, tau: int) -> float:
        ...
        for pos in range(seq_len):
            ...
            if x_token == x_prime_token:
                continue
            if x_token == self.model.mask_index:
