...
class Diffusion(L.LightningModule):
    def __init__(
        ...
        if tokenizer is None:
            ...
            possible_splits = current_file.parent / "tokenizer" / "new_splits.txt"
            
            if possible_vocab.exists() and possible_splits.exists():
                self.tokenizer = SMILES_SPE_Tokenizer(str(possible_vocab), str(possible_splits))
            else:
                raise ValueError(
                    f"Tokenizer is None and could not find tokenizer files automatically.\n"
                    f"Looked for: {possible_vocab}\n"
                    f"Please provide a tokenizer or ensure tokenizer files exist."
                )
        else:
    ...
    def forward(self, zt, attn_mask, sigma):
        ...
        # Use new torch.amp.autocast API (device-agnostic)
