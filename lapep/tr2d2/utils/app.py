...
class PeptideAnalyzer:
    ...
    def is_peptide(self, smiles):
        ...
        if not RDKIT_AVAILABLE:
            # Fallback: simple regex-based check for peptide bonds
            # Look for NC(=O) pattern (peptide bond)
            if re.search(r'NC\(=O\)', smiles):
                return True
            # Look for N(C)C(=O) pattern (N-methylated peptide bond)
            if re.search(r'N\(C\)C\(=O\)', smiles):
                return True
            return False
        
        # Use rdkit for more accurate detection
